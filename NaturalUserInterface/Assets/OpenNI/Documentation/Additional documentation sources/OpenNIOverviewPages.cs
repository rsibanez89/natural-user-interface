...
@see The @subpage NISkeletonPrefab and the @ref SkeletonBaseObjects module.
*/
...
- <b>Documentation</b> - This directory holds the main documentation for the package. This includes:
    ...
    - <b>ExampleRecord.xml</b>: This is an example for playing a recording. See 
      @ref OpenNIRecordAndPlayback "Recording and playing back sensor data" for more information.
...
save the user to player mapping, then the object containing the NIUsersToPlayerMapper should also 
...
-# <b>Use the NiBackRecorder sample</b>: niBackRecorder is a command line tool, which stores frames
   in memory in a cyclic buffer. Clicking "D" sends a request to dump this cyclic buffer to an ONI file.
   In effect, it saves the last certain number of seconds, according to how it has been configured. This
   means that a developer can run the recorder in the background, run the Unity program and then save the
   result. To do this do the following:
    - Run the NIViewer sample (this is by default in "C:\Program Files\OpenNI\Samples\Bin\Release" on a
      32 bit machine and in "C:\Program Files (x86)\OpenNI\Samples\Bin\Release" on a 64 bit machine.
...
Now that we have a recording, we want to play it. To do so we need to write an XML file which uses it
For simplicity, a sample XML file is provided (ExampleRecord.xml in the documentation directory). <br>
The main change which needs to be done in the XML file is to change the filename which holds the recorded
data (if one uses extensions then more production nodes might be required).
@note this works only for implementations which use depth as the depth node is the one being recorded and
played back.

In the OpenNI configuration object of the @ref OpenNISettingsPrefab (or directly from the 
@ref OpenNISettingsManager script) the XML file name needs to be set. also the 
checkbox for "Use playback Mode" needs to be checked.
...
feet is high when making the calibration pose, one can assume that the floor is just below the feet (This
is known as pinning to the ground). Various techniques can be used to make this work well. For example,
if jumping is intended, then one cannot pin to the ground all the time.
...
One way to find out this rotation is assuming that the user stands erect while in calibration pose (or 
any game-specific gesture in which the user stands erect). This means that the vector between the torso
and the head is parallel to the floor's normal. A very rough example of how to do this is supplied in
@ref NIFloorNormalCalculator.<br> 

...
The goal of @ref NIGUI is to provide an easy to use GUI creator that uses Natural Interactions. This
is part of the Utilities module rather than part of the @ref OpenNIPackage, as this is more of
an example of how to implement this rather than a fully fleshed solution. That said, %NIGUI is built
on the notion of providing an almost transparent wrapper to Unity's own GUI. The basic idea is to
remain true to <A HREF="http://unity3d.com/support/documentation/Components/GUI%20Scripting%20Guide.html"> 
Unity's GUI scripting guide </A>, replacing the regular <A HREF="http://unity3d.com/support/documentation/ScriptReference/GUI.html"> 
Unity's GUI class </A> with the @ref NIGUI class. %NIGUI does not implement all elements of GUI. 
This is for two reasons; the first is that some elements are simply not required, for example, a label
will work exactly the same as there is no interaction with it. Therefore, there is no reason for 
NIGUI to rewrite it. The second is that some elements are more problematic to implement and 
therefore are left to the user to implement. In this case, the various GUI windows options were not 
implemented. <br>
...
assumes you have created the following axes in NIInput: NIGUI_X to control the 'x' axis, 
NIGUI_Y to control the 'y' axis and NIGUI_CLICK to click. If you want to control NIGUI both
from the Natural Interactions input and from regular input, such as mouse, the appropriate axes must be
defined in InputManager.<br>

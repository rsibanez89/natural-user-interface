...
public class NIDepthmapViewerUtility : NIMapViewerBaseUtility 
{
	...
	public Color DepthMapColor = new Color(226,0,255);
	...
	// valor maximo soportado por kinect
	public short depthMin = 409;
	// valor minimo soportado por kinect
	public short depthMax = 9757;
	...
	protected override bool InitTexture(out Texture2D refText, out int xSize, out int ySize)
    {
        ...
        {
            m_context.m_Logger.Log("No depth", NIEventLogger.Categories.Errors, NIEventLogger.Sources.BaseObjects);
            return false;
        }
        ...
        if(m_factor<=0)
        {
            m_context.m_Logger.Log("Illegal factor", NIEventLogger.Categories.Errors, NIEventLogger.Sources.Image);
            return false;
        }
	...
	private Color getColor(short distanciaReal)
	{   
		if (distanciaReal < 1500)//short distance
			return getColor(distanciaReal, 409, 1499, Color.red, Color.green);
		if (distanciaReal < 5000)//good distance
			return getColor(distanciaReal, 1500, 4999, Color.green, Color.blue);
		if (distanciaReal < 7000)//far distance
			return getColor(distanciaReal, 5000, 6999, Color.blue, Color.black);
		//not detected
		return new Color(0, 0, 0);
	}
	
	private Color getColor(short distanciaReal, short min, short max, Color inicial, Color final) {
		float rinic = inicial.r;
		float ginic = inicial.g;
		float binic = inicial.b;

		float rfin = final.r;
		float gfin = final.g;
		float bfin = final.b;

		short largoIntervalo = (short)(max - min);
		short distancia = (short)(distanciaReal - min);

		float r = ((rinic * (largoIntervalo - distancia)) + (rfin * distancia))/ largoIntervalo;
		float g = ((ginic * (largoIntervalo - distancia)) + (gfin * distancia))/ largoIntervalo;
		float b = ((binic * (largoIntervalo - distancia)) + (bfin * distancia))/ largoIntervalo;

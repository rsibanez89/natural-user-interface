...
public class NIUsermapViewerUtility : NIMapViewerBaseUtility 
{
	...
	protected override bool InitTexture(out Texture2D refText, out int xSize, out int ySize)
    {
        ...
        {
            m_context.m_Logger.Log("Invalid users node", NIEventLogger.Categories.Errors, NIEventLogger.Sources.Skeleton);
            return false;
        }
        if(m_factor<=0)
        {
            m_context.m_Logger.Log("Illegal factor", NIEventLogger.Categories.Errors, NIEventLogger.Sources.Skeleton);
            return false;
        }

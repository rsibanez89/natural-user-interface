...
abstract public class NIWrapperObject
{
    ...
    {
        if(logger!=null)
            logger.Log("In "+this.GetType()+ ":Init("+logger+")", NIEventLogger.Categories.Initialization, NIEventLogger.Sources.BaseObjects);
        m_Logger = logger;
        return true;
	}
    ...
    {
        Log("Disposing of " + GetType(), NIEventLogger.Categories.Initialization, NIEventLogger.Sources.BaseObjects);
    }
